...
Use this page to draw and review the Revision B power circuit. Revision B
replaces the `SY6280AAC` USB limiter and `ETA6002E8A` charger with one TI
`BQ24073RGTR`. Use it only with an approved, protected, single-cell 4.2 V
...
Use the manufacturer's RGT 16-pin, 3 mm by 3 mm exposed-pad footprint. The
specified part is TI `BQ24073RGTR`, LCSC
[`C15220`](https://www.lcsc.com/product-detail/C15220.html). Verify the current
TI data sheet, stock, and land pattern before release. Do not reuse either
Revision A footprint.
...
The table is the schematic source of truth. Keep `USB_VBUS_RAW` and the boosted
`+5V` rail separate. Also keep `BAT_PROTECTED` separate from `VSYS`; the
BQ24073 manages the connection between them.
...
Use a high-efficiency LED and start with 4.7 kOhm at 3.0 V. `CHG` is low while
charging and high-impedance when charging is complete or disabled. Do not add
a separate pull-up until the combined current has been checked.
...
3. With battery and USB present, measure total VBUS current at idle, maximum
   backlight, and maximum backlight while charging. The T470 backlight alone
   drew 595 mA at 80% duty and 620 mA at 85% duty with 50 kHz PWM; at 90%, the
   Analog Discovery supply shut down. A USB500 source cannot supply those
   measured loads by itself. Define and test the permitted duty cycle, charging
   behavior, and any battery-assisted load sharing for every power state.
