...
This page assigns the HolyIOT GPIOs on the shared Revision B core. A 60-contact
FFC connects the core to either a passive T430 adapter or a passive T470
adapter. Its footprint and pinout are defined in
[`universal-connector.md`](universal-connector.md). This document is
the source of truth for the module end of those nets. Model-specific connector
pins belong in the adapter pages.

The T470 straight 40-pin connector is defined in
[`connector.md`](../../t470/revision-b/connector.md).

Install only one adapter. Connecting both keyboards would place their matrices
in parallel. HolyIOT pad numbers use the manufacturer's top view.
...
channels defined in
[`universal-connector.md`](universal-connector.md#trackpoint-iv-reset-polarity-and-driver).
...
- Implement the shared core-board charger and power path according to
  [`power-path.md`](power-path.md); it replaces the Revision A
